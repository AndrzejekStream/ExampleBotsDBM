...
void UpdateDrawdownStatus()
{
   ...
   if(lastDay == 0 || TimeDay(lastDay) != t.day)
   {
      dayStartEquity = AccountInfoDouble(ACCOUNT_EQUITY);
      consecutiveLosses = 0;
      lastDay = now;
      if(tradingDisabled && disableReason == "Daily DD limit" )
      {
   ...
   if(lastWeek == 0 || (int)(TimeDayOfYear(lastWeek) / 7) != weekNumber)
   {
      weekStartEquity = AccountInfoDouble(ACCOUNT_EQUITY);
      lastWeek = now;
      if(tradingDisabled && disableReason == "Weekly DD limit")
      {
         tradingDisabled = false;
         disableReason = "";
      }
